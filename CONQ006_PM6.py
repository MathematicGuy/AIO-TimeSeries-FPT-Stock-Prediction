...
#
#  - Pipeline:
#       1) [PREPROCESS] sanity checks + clean time axis + remove duplicates/outliers
#       2) FE: STL + OHLCV + patterns
#       3) Math backbone (trend trên log(close)):
...
#       6) Pricing-layer (regime-aware, pure FPT):
...
#       7) FINAL MODEL:
#           - [TRAIN] Train XGB residual trên full FPT_train
#           - [FORECAST] Hybrid raw path H=100 ngày
#           - [FORECAST] Áp pricing-best để ra BASE path
#           - [FORECAST] TREND (linear price), UNCERT analytic, ensemble 70–25–5
#
#  - Outputs:
#       * FPT_forecast.csv
...
def build_modeling_df(
    ...
    """
    [FE + LABEL] – dùng cho TRAIN (cả cutoff + final model).
...
def train_xgb_on_dfmodel(
    ...
    # residuals full để tính sigma / mean cho Risk Expert
...
def apply_pricing_on_raw_path(
    ...
    """
    [FORECAST + CV] Áp pricing-layer lên chuỗi raw_rets hybrid:
...
def build_cv_cutoffs(
    ...
    """
    [CV-EVAL] Chọn cutoff: cuối các tháng [3, 6, 9, 12] mỗi năm trong [start_year, end_year],
...
def build_uncertainty_band(
    ...
    """
    [FORECAST] analytic band ~ random walk với sigma = resid_std.
    Dùng cho Risk Expert.
    """
    H = len(base_path)
...
def train_final_base_model(
    ...
    """
    [TRAIN] Train XGB residual cuối cùng trên toàn bộ FPT_train.
    """
